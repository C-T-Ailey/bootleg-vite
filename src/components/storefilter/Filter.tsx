...
export default function Filter({selectedSource, setSelectedSource, selectedFormat, setSelectedFormat, selectedSort, setSelectedSort}:FilterProps) {
  ...
  return (
    <div id="filter-wrapper" className="text-center">
      <div id='toggle' className={`${buttonStyle} border-2 rounded-md w-fit mx-auto mb-2 cursor-pointer`} onClick={() => setFilterToggle(!filterToggle)}>Filter</div>
      <div className={`${filterToggle === true ? "h-40" : "h-0"} overflow-hidden transition-height ease duration-[700ms]`}>
